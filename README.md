...
Lo stato parte sempre da **Non pagato**. La cella è un menu a tendina con due sole voci — **Non pagato** e **Pagato** — che in Google Fogli appare come un pulsante da cliccare: rosso finché non paga, verde quando confermi. Non devi scrivere niente a mano.

> Se il foglio ha già delle righe registrate prima di questa modifica, esegui una volta la funzione **`sistemaFoglio`** dall'editor di Apps Script: mette tendina e colori su tutte le righe esistenti e converte i vecchi "Da verificare" in "Non pagato".

### Scadenze pagamento
- Tutti i metodi: **12 agosto 2026**
- Contanti: **10 agosto 2026** (poi non è più accettato)

Le date stanno in `SCADENZA` e `SCADENZA_CONTANTI`, presenti sia in `script.js` (sito) sia in `Codice.gs` (mail). Se le cambi, cambiale in entrambi.
...
## 8. Cambiare account Google

### Cosa NON esiste in questo progetto

Prima di cercarle: **non ci sono** credenziali OAuth, API key, service account, variabili d'ambiente o webhook. Verificato: zero occorrenze nel codice. Il progetto non ne ha bisogno, perché Apps Script gira *dentro* l'account che lo possiede e usa direttamente i suoi permessi.

Quindi non c'è niente da revocare o rigenerare. **I valori legati all'account sono due, e basta:**

| Valore | File | Cos'è |
|---|---|---|
| `SHEET_ID` | `Codice.gs`, riga 12 | l'ID del foglio |
| `SCRIPT_URL` | `script.js`, riga 10 | l'indirizzo del Web App |

Sono entrambi in cima al rispettivo file, dentro il blocco `CONFIG`. Il trasloco è cambiare questi due.

### Procedura (15 minuti)

**Sul nuovo account:**

1. **Il foglio.** Due strade:
   - *Portare le prenotazioni esistenti:* dal vecchio account apri il foglio → **Condividi** → aggiungi il nuovo account come **Editor**. Poi entra col nuovo account, apri il foglio → **File › Crea una copia** e salvala nel suo Drive. La copia appartiene al nuovo account.
   - *Ripartire da zero:* vai su **sheets.new** col nuovo account. Foglio e intestazioni si creano da soli alla prima prenotazione.
2. Copia il nuovo **ID** dall'URL (il pezzo tra `/d/` e `/edit`).
3. **Apps Script.** Apri il nuovo foglio → **Estensioni › Apps Script** → incolla tutto `Codice.gs`.
4. Sostituisci `SHEET_ID` con il nuovo ID. Salva.
5. Esegui la funzione **`test`** → autorizza col nuovo account. La mail di prova ora parte dal *nuovo* indirizzo: controlla che arrivi.
6. **Distribuisci › Nuova distribuzione › ⚙️ App web** → *Esegui come:* **Io** — *Chi ha accesso:* **Chiunque** → copia il nuovo URL `/exec`.

**Sul sito:**

7. In `script.js` sostituisci `SCRIPT_URL` con il nuovo URL. Salva e ricarica il sito su Cloudflare.
8. Fai una prenotazione di prova vera dal sito: deve comparire la riga nel *nuovo* foglio e arrivare la mail dal *nuovo* indirizzo.

**Solo quando il punto 8 funziona, sul vecchio account:**

9. **Distribuisci › Gestisci distribuzioni › ⚙️ › Archivia**. Così il vecchio endpoint smette di rispondere e non ti ritrovi prenotazioni sparse su due fogli.
10. Facoltativo: revoca l'accesso dello script da [myaccount.google.com/permissions](https://myaccount.google.com/permissions).

### Da sapere

- **Il mittente cambia:** le mail partiranno dall'indirizzo del nuovo account. Il nome visualizzato resta "Staff HACE FALÒ" (`NOME_MITTENTE` in `Codice.gs`).
- **L'ordine conta:** non archiviare il vecchio Web App prima che il nuovo funzioni, o il sito resta scollegato.
- **Limite mail:** Gmail gratuito 100 al giorno, Workspace 1500. Riparte da zero sul nuovo account.
- **Non toccare** PayPal, IBAN, video e testi: non dipendono dall'account Google.
